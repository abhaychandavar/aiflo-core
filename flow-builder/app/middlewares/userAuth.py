...
async def validate_user_access(request: Request):
    try:
        ...
        if isinstance(e, APP_ERROR):
            return api_helpers.response_handler(data=None, error=e)
        e = APP_ERROR(code='app/internal-error', status_code=StatusCode.UNAUTHORIZED, message="Unauthorized access")
        return api_helpers.response_handler(data=None, error=e)
